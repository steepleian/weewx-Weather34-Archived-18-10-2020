...
def replace_variables(config_dict, var, default = None):
...
def parse_settings_file(filename):
    ...
    try:
        ...
        with open(filename, "r" ) as read_file:
            config_dict = dict(remove_spaces(x) for x in [functools.reduce(lambda a, kv: a.replace(*kv), replchars, line) for line in read_file if "=" in line])
               
    except Exception as err:
        logerr("Failed to open config file or create dictionary: %s, Error: %s" % (filename, err))
    return config_dict

def do_file_transfer(mode, path, conn, address):
    try:
        if mode == 'rsync':
...
class ForecastData():    
    def __init__(self, filename):
        config_dict = parse_settings_file(filename)
        if len(config_dict) == 0:
            return
        service_str = replace_variables(config_dict, "services")
        if not service_str == None and len(service_str) > 0:
            service_dir = replace_variables(config_dict, "servicesWriteDir") 
            for service in service_str.split("."):
                try:
                    service_filename = service_dir + service + ".txt" if not service_dir == None else None
                    thread = threading.Thread(target = self.get_website_data, args = (service, replace_variables(config_dict, service + "_url"), replace_variables(config_dict, service + "_filename", service_filename), replace_variables(config_dict, service + "_interval", "3600"), replace_variables(config_dict, service + "_header", "'User-Agent':'Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6_4; en-US) AppleWebKit/534.3 (KHTML, like Gecko) Chrome/6.0.472.63 Safari/534.3'").split(":")))
                    thread.daemon = True
                    thread.start()
                except Exception as err:
...
class CloudCover():
    def __init__(self, weewx_dict):
        ...
        settings_dict = parse_settings_file(weewx_dict.get('Weather34WebServices').get('filename'))
        ...
        try:
            ...
            thread = threading.Thread(target = self.calculate_cloud_cover, args = (settings_dict,))
            thread.daemon = True
            thread.start()
    ...
    def calculate_cloud_cover(self, settings_dict):
        try:
            lat = replace_variables(settings_dict, 'lat')
            lon = replace_variables(settings_dict, 'lon')
            url1 = '"'+replace_variables(settings_dict, 'cc1_url')+'"'
            url2 = '"'+replace_variables(settings_dict, 'cc2_url')+'"'
            file1 = replace_variables(settings_dict, 'cc1_filename')
            file2 = replace_variables(settings_dict, 'cc2_filename')
            time_interval = int(replace_variables(settings_dict, 'cc_interval', 600))
    ...
    def listen_data_requests(self, port, host):
        ...
        while True:
            try:
                ...
                elif not host.replace('.', '').isnumeric():
    ...
    def execute_report(self, args, conn, address):
        ...
        try:
            ...
            with weewx.manager.DBBinder(config_dict) as db_binder:
                ...
                do_file_transfer(args[4], os.path.join(html_root,args[2].split(".tmpl")[0]), conn, address)
        except Exception as e:
            logerr("Webserver: " + str(e))
...
class Weather34RealTime(StdService):
    ...
    def __init__(self, engine, config_dict):
        ...
        self.filename = d.get('filename', '/var/www/html/weewx/weather34/w34realtime.txt')
        ...
        try:    
            self.webserver_addresses = Webserver(config_dict).webserver_addresses
    ...
    def handle_new_archive(self, event):
        if self.prev_archive_time + 50 < time.time():
            self.prev_archive_time = time.time()
            thread = threading.Thread(target=self.do_rsync_transfer, args=(os.path.join(self.config_dict['StdReport'].get('w34Highcharts').get('HTML_ROOT'), 'json/'),))
            thread.daemon = True
            thread.start()
        if self.cc != None:
            self.cc.update_cloud_cover(event)

    def do_rsync_transfer(self, path):
        if len(self.webserver_addresses) > 0:
            for web_address in self.webserver_addresses:
                if not web_address.replace('.', '').isnumeric():
                    web_address = socket.gethostbyname(web_address)
                do_file_transfer("rsync", path, None, web_address) 
    ...
    def write_data(self, data):
        with open(self.filename, 'w') as f:
            ...
            f.write("\n")
        thread = threading.Thread(target=self.do_rsync_transfer, args=(self.filename,))
        thread.daemon = True
        thread.start()
